...
WORK_DIR="$(mktemp -d "$TEMP_ROOT/ofxtvosboost-cocoapods.XXXXXX")"
cleanup() {
    [[ -n "$WORK_DIR" && -d "$WORK_DIR" && ! -L "$WORK_DIR" ]] || return
    case "$WORK_DIR" in
        "$TEMP_ROOT"/ofxtvosboost-cocoapods.*) ;;
        *) return ;;
    esac
    rm -rf "$WORK_DIR"
}
...
abstract_target 'DownloadTest' do
  pod 'ofxtvOSBoost', '$BOOST_VERSION'
end
EOF

echo "Downloading ofxtvOSBoost $BOOST_VERSION through the public CocoaPods CDN"
if ! (
    cd "$WORK_DIR"
    CP_HOME_DIR="$WORK_DIR/cocoapods-home" pod install --repo-update --no-ansi
); then
    echo "CocoaPods could not install ofxtvOSBoost $BOOST_VERSION." >&2
    exit 1
fi
...
echo "PASS: CocoaPods downloaded ofxtvOSBoost $BOOST_VERSION and validated boost.xcframework."
