...
TARGET="$INSTALL_ROOT/tvos/boost.xcframework"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/ofxtvosboost-install.XXXXXX")"
...
if [[ "$REQUESTED_VERSION" == "latest" ]]; then
    ...
    release_url="$(curl --fail --location --silent --show-error \
        ...
        https://github.com/danoli3/ofxtvOSBoost/releases/latest)"
...
ASSET="ofxtvOSBoost-${BOOST_VERSION}.tar.gz"
RELEASE_ROOT="https://github.com/danoli3/ofxtvOSBoost/releases/download/$release_tag"
...
SOURCE="$WORK_DIR/ofxtvOSBoost-${BOOST_VERSION}/libs/boost/tvos/boost.xcframework"
plutil -lint "$SOURCE/Info.plist" >/dev/null

mkdir -p "$INSTALL_ROOT/tvos"
if [[ -e "$TARGET" ]]; then
    [[ "$TARGET" == "$REPO_ROOT/libs/boost/tvos/boost.xcframework" ]] || exit 1
    rm -rf "$TARGET"
fi
cp -R "$SOURCE" "$TARGET"
rm -rf "$INSTALL_ROOT/cmake" "$INSTALL_ROOT/pkgconfig"
cp -R "$WORK_DIR/ofxtvOSBoost-${BOOST_VERSION}/libs/boost/cmake" "$INSTALL_ROOT/"
cp -R "$WORK_DIR/ofxtvOSBoost-${BOOST_VERSION}/libs/boost/pkgconfig" "$INSTALL_ROOT/"
