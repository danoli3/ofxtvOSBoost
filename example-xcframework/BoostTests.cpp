...
{
    const boost::filesystem::path path("/tmp/ofxtvOSBoost/example.txt");
...
bool testBoost172Updates(std::string &detail)
{
    try {
        boost::filesystem::file_size(
            boost::filesystem::path("/ofxtvosboost/nonexistent/1.72"));
...
bool testBoost174Features(std::string &detail)
{
    ...
    const boost::filesystem::path directory =
        boost::filesystem::path(temporaryRoot) / "ofxtvosboost-1.74-smoke";
...
bool testBoost175Features(std::string &detail)
{
    ...
    const bool jsonPassed = object.at("name").as_string() == "Boost" &&
        ...
        serializedJson.find("tvOS") != std::string::npos;
...
bool testBoost182Json(std::string &detail)
{
    ...
    value.set_at_pointer("/platform/name", "tvOS");
    ...
    return value.at_pointer("/platform/name") == "tvOS" && hash != 0 &&
...
bool testBoost182Url(std::string &detail)
{
    ...
    formatted.set_params({{"version", "1.82"}, {"target", "tvOS"}});
    ...
    return formatted.encoded_path() == "/release/C++%2020" &&
        formatted.encoded_query() == "version=1.82&target=tvOS" &&
        !view.empty();
}
...
bool testBoost183Filesystem(std::string &detail)
{
    ...
    const boost::filesystem::path directory =
        boost::filesystem::path(temporaryRoot) / "ofxtvosboost-1.83-smoke";
...
{
    const auto parsed = boost::urls::parse_uri(
        "http://[fe80::1%25en0]/status?version=1.83&target=tvOS");
    const auto query = boost::urls::parse_query("version=1.83&target=tvOS");
...
bool testBoost184Redis(std::string &detail)
{
    ...
    request.push("SET", "ofxtvOSBoost", "1.84.0", "EX", 84);
...
{
    const boost185_test::PrivateJsonRecord original(85, "tvOS");
    ...
    return restored.version() == 85 && restored.platform() == "tvOS";
}
...
bool testBoost186FilesystemWeaklyCanonical(std::string &detail)
{
    const boost::filesystem::path base =
        boost::filesystem::temp_directory_path() / "ofxtvosboost-186" / "child";
...
bool testBoost192FlatContainerRanges(std::string &detail)
{
    const std::vector<std::pair<int, int>> input{{1, 19}, {2, 92}};
    boost::unordered_flat_map<int, int> values(boost::unordered::from_range, input);
    const std::vector<std::pair<int, int>> extra{{2, 0}, {3, 192}};
    values.insert_range(extra);
    detail = "Unordered non-concurrent flat map supports C++20 range construction and insertion";
    return values.size() == 3 && values.at(1) == 19 && values.at(2) == 92 && values.at(3) == 192;
}

bool testBoost192ConcurrentFlatContainerRanges(std::string &detail)
{
    const std::vector<std::pair<int, int>> input{{1, 19}, {2, 92}};
    boost::concurrent_flat_map<int, int> values(boost::unordered::from_range, input);
    const std::vector<std::pair<int, int>> extra{{2, 0}, {3, 192}};
    int duplicates = 0;
    values.insert_range_or_visit(extra, [&duplicates](auto &entry) {
        if (entry.first == 2 && entry.second == 92) ++duplicates;
    });
    int sum = 0;
    values.cvisit_all([&sum](const auto &entry) { sum += entry.second; });
    detail = "Unordered concurrent flat map range insertion visits duplicates and preserves values";
    return values.size() == 3 && duplicates == 1 && sum == 303;
}
...
const std::vector<BoostTestCase> &boostTestCases()
{
    static const std::vector<BoostTestCase> tests{
        ...
        {"Boost 1.92 flat container ranges", testBoost192FlatContainerRanges},
        {"Boost 1.92 concurrent flat container ranges", testBoost192ConcurrentFlatContainerRanges},
